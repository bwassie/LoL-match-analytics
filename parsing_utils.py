...
def get_tables(match, alias,joins, anti_joins=[]):
    tables = []
    join_str = ""
    anti_join_str = ""
    if len(joins) > 0:
...
def create_table(db,name,alias,tables):
    match = db
    match_cur = match.cursor()
    try:
...
def get_matches_from_table(db,name,year='2017',league="NALCS",player="team"):
    match = db
    match_cur = match.cursor()
    ...
    if player == "team":
        query = """SELECT * FROM '{}' AS 'matches' INNER JOIN '{}' AS 
            'table' ON 'table'.Team = 'matches'.team WHERE 'matches'.league = "{}" 
            AND 'matches'.player LIKE '%team%';""".format(table,name,league)
        matches = pd.read_sql_query(query, match).fillna(np.nan)
    else:
        query = """SELECT * FROM '{}' AS 'matches' INNER JOIN '{}' AS 
            'table' ON 'table'.Team = 'matches'.team WHERE 'matches'.league = "{}" 
            AND 'matches'.player NOT LIKE '%team%';""".format(table,name,league)
        matches = pd.read_sql_query(query, match).fillna(np.nan)
    return matches

def get_matches_master(dct,alias,joins,anti_joins,name,year,league):
    match,aliases = get_dbs(dir1)
    tables = get_tables(match,alias,joins,anti_joins)
    create_table(match,name,alias, tables)
    matches = get_matches_from_table(match,name,year,league,joins[1])
    return matches